...
module('Acceptance: Translation', function(hooks) {
  ...
  test('English translations are correct', async function(assert) {
    ...
    assert.equal(find('div#translated-with-bound-args').textContent, '1000 frogs');
  });
  ...
  test('Changing locale changes text', async function(assert) {
    ...
    assert.equal(find('div#translated-with-bound-args').textContent, 'thai 1000 frog');
  });
  ...
  test('Changing bound params changes text', async function(assert) {
    ...
    assert.equal(find('div#translated-with-bound-args').textContent, '5000 frogs');
  });
  ...
  test('Markup is allowed in translation keys but substitutions are escaped', async function(assert) {
    ...
    assert.equal(findAll('div#translated-with-markup b').length, 1, 'there should be a bold tag');
    assert.equal(findAll('div#translated-with-markup-malicious script').length, 0,
